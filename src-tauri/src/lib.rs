...
mod app_init {
    ...
    pub fn generate_handlers() -> impl Fn(tauri::ipc::Invoke<tauri::Wry>) -> bool + Send + Sync + 'static {
        tauri::generate_handler![
            ...
            cmd::get_xray_core_status,
            cmd::check_xray_core_update,
            cmd::install_xray_core,
            cmd::remove_xray_core,
            cmd::set_xray_relay_enabled,
            cmd::export_xray_config,
            cmd::export_runtime_config,
            cmd::get_xray_config_path,
            cmd::notify_relay_state,
    ...
    let builder = app_init::setup_plugins(tauri::Builder::default())
        .setup(|app| {
            ...
            fn log_setup_panic(stage: &str, panic: Box<dyn std::any::Any + Send>) {
                ...
                eprintln!("[celestial] panic during app setup ({stage}), continuing in degraded mode: {msg}");
                logging!(
                    error,
                    Type::Setup,
    ...
    mod event_handlers {
        ...
        pub fn handle_ready_resumed(_app_handle: &AppHandle) {
            ...
            if let Some(window) = _app_handle.get_webview_window("main") {
                let _ = window.set_title("Celestial");
            }
        }
    ...
    app.run(|app_handle, e| match e {
        ...
        tauri::RunEvent::ExitRequested { api, code, .. } => {
            if module::lightweight::is_in_lightweight_mode() && !handle::Handle::global().is_exiting() {
                ...
                api.prevent_exit();
                if !handle::Handle::global().is_exiting() {
                    // Must finish before the loop tears down, otherwise cleanup
                    // races the process going away.
