//! Owner identity and credentials for the privileged service.
//!
//! Carried over from upstream whole rather than trimmed to today's callers:
//! the private-file, SDDL and identity helpers below are what the next stage
//! (owner-scoped app dirs and the local server) wires up, and re-deriving the
//! Win32 ACL handling later is more error-prone than keeping it intact.
#![allow(dead_code)]
...
mod windows_owner {
    ...
    use celestial_service_ipc::OWNER_TOKEN_FILE_NAME;
    use std::ffi::c_void;
...
mod tests {
    ...
    use celestial_service_ipc::OwnerIdentity;
