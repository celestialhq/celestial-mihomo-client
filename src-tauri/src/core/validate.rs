...
/// 验证失败的分类。之前调用方靠 `starts_with`/`contains` 去猜错误文本，
/// 消息一改通知就悄悄错位；分类在这里做一次，调用方只匹配枚举。
...
/// 验证结果。关键是 `Skipped`/`Busy` 不再冒充成功——之前它们都返回
/// `(true, "")`，调用方无法区分「验证通过」和「压根没验证」。
...
impl CoreConfigValidator {
    ...
    pub async fn validate_config_file_outcome(
        ...
        // `is_script_file` 自己就按扩展名快速判定 .js，无需在这里重复。
    ...
    async fn validate_config_internal_outcome(config_path: &str) -> Result<ValidationOutcome> {
        ...
        // Deliberately `Valid`, not `Skipped`: `Skipped` reports `is_valid() == false`,
        ...
        #[cfg(not(any(target_os = "android", target_os = "ios")))]
        {
            ...
            if has_error {
                ...
                // No exit code means the process was killed rather than rejecting
                // the config, which is a different failure for the caller.
