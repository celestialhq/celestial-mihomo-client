...
impl Logger {
    ...
    pub async fn init(&self) -> Result<()> {
        ...
        *self.log_level.write() = log_level;
        ...
        #[cfg(not(any(feature = "tauri-dev", feature = "tokio-trace")))]
        {
            ...
            let logger = flexi_logger::Logger::with(log_spec)
                ...
                .format(celestial_logger::console_format)
                .format_for_files(celestial_logger::file_format_with_level)
                .rotate(
        ...
        *self.sidecar_file_writer.write() = Some(sidecar_file_writer);
        ...
        std::panic::set_hook(Box::new(move |info| {
            // Capture both common panic payload types — `panic!("{}", x)` produces a
            // String payload, which the &str-only downcast logged as "unknown".
            // This global hook only covers panics after logger init; earlier setup
            // panics are caught in lib.rs.
            let payload = info
                .payload()
                .downcast_ref::<&str>()
                .map(|s| (*s).to_string())
    ...
    fn generate_sidecar_writer(&self) -> Result<FileLogWriter> {
        ...
        .format(celestial_logger::file_format_without_level)
        .rotate(
            Criterion::Size(log_max_size * 1024),
            flexi_logger::Naming::TimestampsCustomFormat {
                current_infix: Some("latest"),
