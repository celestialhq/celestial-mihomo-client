// System-wide OS proxy configuration (Windows registry / macOS networksetup /
// Linux gsettings via the `sysproxy` crate). Third-party Android/iOS apps
// cannot set a system-wide proxy, so on those platforms `Sysopt` is a no-op
// stub with the same public API — only the VPN/TUN connection mode exists
// there.
