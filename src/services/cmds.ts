...
/** Returns false when the backend rejected and restored the file — callers
 *  should re-read it rather than keep showing the rejected text. */
...
export async function getProcessIcon(processPath: string) {
  return invoke<string | null>('get_process_icon', { processPath })
}
