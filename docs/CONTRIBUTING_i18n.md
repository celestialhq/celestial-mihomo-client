...
- If you touch backend copy, edit the matching YAML file in `crates/celestial-i18n/locales/<lang>.yml`.
...
Native UI strings (tray menu, notifications, dialogs) use `rust-i18n` with YAML bundles stored in `crates/celestial-i18n/locales/<lang>.yml`. These files are completely independent from the frontend JSON modules.
...
- Rust code resolves the active language through the `celestial-i18n` crate (`crates/celestial-i18n/src/lib.rs`). No additional build step is required after editing YAML files; `tauri dev` and `tauri build` pick them up automatically.
...
4. If the backend should expose the language, create `crates/celestial-i18n/<new-lang>.yml` and translate the keys used in existing YAML files.
